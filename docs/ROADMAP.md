...
- [ ] **15.1** Définir le modèle de données des annotations : table `annotations` liée à une fiche (contenu de la note, ancre = citation exacte + contexte avant/après + offset indicatif, horodatages création/mise à jour). Cascade uniquement sur suppression réelle de la fiche ; conservation lors d'un simple passage `active = false`. Générer et appliquer la migration.
- [ ] **15.2** Créer le module serveur `$lib/server/annotations.ts` (à l'image de `bookmarks.ts`) : création, liste par fiche, mise à jour, suppression ; handle Drizzle explicite, sans import SvelteKit, testable unitairement. Valider au boundary la note (non vide, longueur maximale) et la forme de l'ancre.
