...
- Every code path a change adds or fixes — error branches, validation rejections, 404/duplicate/4xx id paths, entity-decoding cases — and every user-visible deliverable must be exercised by a test in the same change; a passing higher-level test must not mask, nor be relied on to imply, lower-level coverage.
- At least one integration test must exercise the app's real startup against a fresh, unprovisioned database (no out-of-band `db:migrate`/drizzle-kit migrate in the fixture): test fixtures must not perform an initialization step the production app does not run itself, or a green suite will mask a boot-time failure (e.g. "no such table").
...
- Validate and bound the shape of externally supplied values at the server boundary (repo URL scheme, branch/path format, minimum secret strength; cap all persisted strings, and reject empty or non-numeric values in numeric fields explicitly instead of coercing '' to 0) rather than accepting any non-empty string.
- Keep SvelteKit adapter and compiler options in a conventional svelte.config.js, not inline in vite.config.ts.
- Never extract or sanitize HTML with regexes or a default DOMPurify config: parse via the DOM and pass an explicit allow-list (ADD_ATTR target/rel) so attribute values containing '>' and external-link attributes survive.
- Land feature work in reviewable increments (soft cap ~400 changed lines / a handful of files per commit); split independent concerns into separate commits rather than one large multi-file diff.
- Never include changes unrelated to the task (e.g. editor/IDE files like .vscode/settings.json) in a change; gitignore local editor config and keep the working tree scoped to the feature.
- Tests must assert the module's actual output, not fixture constants, and must not parse source with position-dependent regexes (e.g. assuming `actions` is the last top-level declaration).
