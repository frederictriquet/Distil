...
//   - 11.1: /cards lists only active cards belonging to a knowledge base
...
//     title, theme OR content; LIKE wildcards in the query (`%`, `_`) are
//     escaped and matched literally, and multiple terms restrict with AND;
...
//
// The index /cards and the child route /cards/<id> coexist without shadowing:
// /cards renders this list while /cards/<id> renders the consultation page and
// an unknown id still answers 404 (asserted here in the same fixture).
//
...
function buildIsolatedAppCopy() {
	const appDir = fs.mkdtempSync(path.join(os.tmpdir(), 'distil-cards-app-copy-'));
...
async function startApp(env) {
	...
	const harnessDir = fs.mkdtempSync(path.join(os.tmpdir(), 'distil-cards-web-harness-'));
...
describe('cards index at /cards: perimeter, search, filters and navigation (roadmap 11)', () => {
	let app;
	let workDir;
	let dbPath;
	let cookie;
	...
	before(async () => {
		workDir = fs.mkdtempSync(path.join(os.tmpdir(), 'distil-cards-'));
	...
	test('an unauthenticated request to /cards is redirected by the access guard (task 3.3 covers new routes)', async () => {
		const res = await fetch(`${app.baseUrl}/cards`, { redirect: 'manual' });
		assert.equal(res.status, 303);
		assert.match(res.headers.get('location'), /^\/login/);
	});

	test('11.1: GET /cards lists only active cards of focused KBs, with title, theme and source', async () => {
	...
	test('11.1: the index /cards and the child route /cards/<id> coexist without shadowing (list renders, unknown id still 404s)', async () => {
		const listRes = await fetch(`${app.baseUrl}/cards`, { redirect: 'manual', headers: { cookie } });
		assert.equal(listRes.status, 200);
		assert.ok(
			extractListedTitles(await listRes.text()).length > 0,
			'the index must render the card list, not be shadowed by the [id] route'
		);

		const missingRes = await fetch(`${app.baseUrl}/cards/999999`, { redirect: 'manual', headers: { cookie } });
		assert.equal(missingRes.status, 404, 'an unknown id under /cards/<id> must still yield 404');
	});
	...
	test('11.2: keyword search matches the title', async () => {
		const res = await fetch(`${app.baseUrl}/cards?${new URLSearchParams({ q: 'SQL Basics' })}`, {
			redirect: 'manual',
			headers: { cookie }
		});
		assert.equal(res.status, 200);
	...
	test('11.2: keyword search matches the theme', async () => {
		const res = await fetch(`${app.baseUrl}/cards?${new URLSearchParams({ q: 'network' })}`, {
			redirect: 'manual',
			headers: { cookie }
		});
		const html = await res.text();
	...
	test('11.2: keyword search matches the content even when title and theme do not', async () => {
		const res = await fetch(`${app.baseUrl}/cards?${new URLSearchParams({ q: 'handshake' })}`, {
			redirect: 'manual',
			headers: { cookie }
		});
		const html = await res.text();
	...
	test('11.2: a keyword search matching nothing in the perimeter shows the "no match" empty state, not an error', async () => {
		const res = await fetch(`${app.baseUrl}/cards?${new URLSearchParams({ q: 'nonexistent-xyz' })}`, {
			redirect: 'manual',
			headers: { cookie }
		});
		assert.equal(res.status, 200);
	...
	test('11.3: theme + level filters combine and exclude a same-theme/level card outside the perimeter (inactive)', async () => {
		const res = await fetch(`${app.baseUrl}/cards?${new URLSearchParams({ theme: 'sql', level: 'beginner' })}`, {
			redirect: 'manual',
			headers: { cookie }
		});
		const html = await res.text();
	...
	test('11.3: KB filter combines with the keyword search', async () => {
		const res = await fetch(`${app.baseUrl}/cards?${new URLSearchParams({ kb: '1', q: 'network' })}`, {
			redirect: 'manual',
			headers: { cookie }
		});
		const html = await res.text();
	...
	test('11.3: a filter combination matching nothing shows the empty state', async () => {
		const res = await fetch(`${app.baseUrl}/cards?${new URLSearchParams({ theme: 'sql', level: 'advanced' })}`, {
			redirect: 'manual',
			headers: { cookie }
		});
		const html = await res.text();
	...
	test('11.3: an out-of-scope or malformed filter value is dropped at the server boundary instead of erroring', async () => {
		const res = await fetch(
			`${app.baseUrl}/cards?${new URLSearchParams({ kb: 'not-a-number', theme: 'does-not-exist', level: 'also-bogus' })}`,
	...
	test('11.3: an out-of-scope KB id (belongs to the unfocused KB) is dropped, not applied', async () => {
		const res = await fetch(`${app.baseUrl}/cards?${new URLSearchParams({ kb: '2' })}`, {
			redirect: 'manual',
			headers: { cookie }
		});
		assert.equal(res.status, 200);
	...
	test('11.4: the search box and filter selects reflect the query params on the resulting page (URL round-trip)', async () => {
		...
		const res = await fetch(`${app.baseUrl}/cards?${query}`, { redirect: 'manual', headers: { cookie } });
		assert.equal(res.status, 200);
		const html = await res.text();
	...
	test('11.4: re-requesting a bare /cards (as after a full reset) shows no filter pre-selected', async () => {
		const res = await fetch(`${app.baseUrl}/cards`, { redirect: 'manual', headers: { cookie } });
		const html = await res.text();
...
async function seedSearchSemantics(dbPath) {
	withRawDb(dbPath, (raw) => {
		// A single focused KB whose active cards form the whole perimeter.
		raw
			.prepare('INSERT INTO knowledge_bases (id, name, repo_url, branch, focus) VALUES (1, ?, ?, ?, 1)')
			.run('KB Search', 'https://example.test/search.git', 'main');
		...
		// '%' wildcard: only the card literally containing "50%" must match q="50%".
		insertCard.run({
			id: 1,
			kbId: 1,
			slug: 'percent-literal',
			title: 'Percent Literal',
			theme: 'promo',
			level: 'beginner',
			sourcePath: 'promo/percent.md',
			content: 'Save 50% on the annual plan.',
			active: 1
		});
		// Contains "50" but not "50%": it would falsely match if '%' were treated
		// as a SQL wildcard instead of an escaped literal.
		insertCard.run({
			id: 2,
			kbId: 1,
			slug: 'percent-decoy',
			...
			theme: 'promo',
			level: 'beginner',
			sourcePath: 'promo/decoy.md',
			content: 'We stock 50 units today.',
			active: 1
		});

		// '_' wildcard: only the card literally containing "a_b" must match q="a_b".
		insertCard.run({
			id: 3,
			kbId: 1,
			slug: 'underscore-literal',
			title: 'Underscore Literal',
			theme: 'syntax',
			level: 'beginner',
			sourcePath: 'syntax/underscore.md',
			content: 'The token a_b is written verbatim.',
			active: 1
		});
		// Contains "axb" but not "a_b": it would falsely match if '_' were treated
		// as a single-character SQL wildcard instead of an escaped literal.
		insertCard.run({
			id: 4,
			kbId: 1,
			slug: 'underscore-decoy',
			title: 'Underscore Decoy',
			theme: 'syntax',
			level: 'beginner',
			sourcePath: 'syntax/decoy.md',
			content: 'The value axb is unrelated.',
			active: 1
		});

		// Multi-term AND: a card matches only if it contains every term. The terms
		// "alpha"/"beta" appear only in content, never in these titles, so title
		// text cannot accidentally satisfy a term.
		insertCard.run({
			id: 5,
			kbId: 1,
			slug: 'both-terms',
			...
			theme: 'topics',
			level: 'beginner',
			sourcePath: 'topics/both.md',
			content: 'This card discusses alpha and beta together.',
			active: 1
		});
		insertCard.run({
			id: 6,
			kbId: 1,
			slug: 'first-term-only',
			title: 'First Term Only',
			theme: 'topics',
			level: 'beginner',
			sourcePath: 'topics/first.md',
			content: 'This card only mentions alpha.',
			active: 1
		});
		insertCard.run({
			id: 7,
			kbId: 1,
			slug: 'second-term-only',
			title: 'Second Term Only',
			theme: 'topics',
			level: 'beginner',
			sourcePath: 'topics/second.md',
			content: 'This card only mentions beta.',
			active: 1
		});
	});
}

describe('cards index at /cards: keyword search semantics — LIKE-wildcard escaping and multi-term AND (roadmap 11.2)', () => {
	let app;
	let workDir;
	let dbPath;
	let cookie;
	...
	before(async () => {
		workDir = fs.mkdtempSync(path.join(os.tmpdir(), 'distil-cards-search-'));
		dbPath = path.join(workDir, 'app.db');
		runMigrate(dbPath);
		await seedSearchSemantics(dbPath);
	...
	/** Run a keyword search and return the matched card titles, sorted. */
	async function searchTitles(q) {
		const res = await fetch(`${app.baseUrl}/cards?${new URLSearchParams({ q })}`, {
			redirect: 'manual',
			headers: { cookie }
		});
		assert.equal(res.status, 200);
		return extractListedTitles(await res.text()).map((t) => t.title).sort();
	}

	test("11.2: a '%' in the query is escaped and matched literally, not as a SQL wildcard", async () => {
		// Only "Percent Literal" contains "50%"; the decoy contains "50" and would
		// match if '%' behaved as a wildcard.
		assert.deepEqual(await searchTitles('50%'), ['Percent Literal']);
	});

	test("11.2: an '_' in the query is escaped and matched literally, not as a single-char SQL wildcard", async () => {
		// Only "Underscore Literal" contains "a_b"; the decoy contains "axb" and
		// would match if '_' behaved as a single-character wildcard.
		assert.deepEqual(await searchTitles('a_b'), ['Underscore Literal']);
	});

	test('11.2: multiple terms restrict with AND — a card matches only if it contains every term', async () => {
		// "alpha beta" requires both terms, so only the card carrying both matches.
		assert.deepEqual(await searchTitles('alpha beta'), ['Both Terms']);
		// A single term still matches every card containing it, which confirms the
		// single-topic cards genuinely exist and were excluded by the AND above.
		assert.deepEqual(await searchTitles('alpha'), ['Both Terms', 'First Term Only']);
	});
});

describe('cards index at /cards: empty perimeter (task 11.1)', () => {
	let app;
	let workDir;
	let dbPath;
	let cookie;
	...
	before(async () => {
		workDir = fs.mkdtempSync(path.join(os.tmpdir(), 'distil-cards-empty-'));
		dbPath = path.join(workDir, 'app.db');
		runMigrate(dbPath);
	...
	test('GET /cards shows the "no cards available" empty state, not an error, when no card is eligible', async () => {
		const res = await fetch(`${app.baseUrl}/cards`, { redirect: 'manual', headers: { cookie } });
		assert.equal(res.status, 200);
		const html = await res.text();
